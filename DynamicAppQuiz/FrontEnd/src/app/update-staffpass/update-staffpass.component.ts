...
@Component({
    selector: 'app-update-staffpass',
    standalone: true,
    imports: [ CommonModule, ReactiveFormsModule],
    templateUrl: './update-staffpass.component.html',
    styleUrl: './update-staffpass.component.css',
})
export class UserProfile1Component implements OnInit {
    ...
    onSubmit(): void {
        if (this.passwordForm.valid) {
            ...
            this.userService.updatePassword1(u_name, password).subscribe(
                response => {
                    alert("Password Changed Successfully")
                    console.log('Password updated successfully');
                    this.router.navigate(['/homeStaff'], { queryParams: { u_name: u_name }})
                },
                error => {
